 ...
 */
class QmlPath final : public QObject {
  Q_OBJECT
  Q_DISABLE_COPY_MOVE(QmlPath)
 ...
 public:
  explicit QmlPath(QObject* parent);
  ~QmlPath() = default;

  [[nodiscard]] bool parse(const QString& obj);

  QQuickItem* evaluate(QQmlApplicationEngine* engine) const;

 private:
  static bool parseInternal(const QChar*& input, qsizetype& size,
                            QList<Data>& blocks);
